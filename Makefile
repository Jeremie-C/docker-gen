...
docker-gen:
	...
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" ./cmd/docker-gen
...
dist: dist-clean
	...
	mkdir -p dist/alpine-linux/armel && CGO_ENABLED=0 GOOS=linux GOARCH=arm GOARM=5 go build -ldflags "$(LDFLAGS)" -a -tags netgo -installsuffix netgo -o dist/alpine-linux/armel/docker-gen ./cmd/docker-gen
...
release: dist
	...
	tar -cvzf docker-gen-alpine-linux-armel-$(TAG).tar.gz -C dist/alpine-linux/armel docker-gen
